...
app.layout = html.Div([
    ...
    ], style={'display': 'flex', 'margin': '20px', 'position':'sticky','top':0, 'zIndex': 999999}),
...
def update_graph(ticker_list_id, time_duration, check_list_id, ticker_list_id_3, mov_av_graph3_id, refresh_button_id):
    ...
    fig2.add_trace(go.Scatter(x = x, y=second_diff5, mode='lines', name='5 minutes', line=dict(color='light blue')))
    ...
    fig2.update_layout( title = ticker_list_id, xaxis_title='Time', yaxis_title='Rate of Change', legend_title='Time Periods', template='plotly', width=graph_width, height=450, 
                       xaxis_range=[max(x)-200, max(x) + 20], yaxis_range=[min(second_diff5[-200:-1]), max(second_diff5[-200:-1])], xaxis=dict(tickvals = custom_tick_vals, ticktext = custom_tick_text, tickangle=-90))
    #yaxis_range=[0, 6]
